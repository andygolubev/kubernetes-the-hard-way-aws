source "amazon-ebs" "ubuntu-kubernetes-the-hard-way-working-node-2" {
  ...
  ami_name = "k8s-working-node-2-{{timestamp}}"
...
build {
  ...
  provisioner "file" { //replace
    sources = ["/tmp/kthw-certs/working-node-2.kubeconfig",
  ...
  provisioner "shell" { // replace
    inline = [
      "sudo cp /etc/kubernetes/certs/working-node-2-key.pem /var/lib/kubelet/",
      "sudo cp /etc/kubernetes/certs/working-node-2.pem /var/lib/kubelet/",
      "sudo cp /etc/kubernetes/certs/ca.pem /var/lib/kubernetes/",
      "sudo cp /etc/kubernetes/config/working-node-2.kubeconfig /var/lib/kubelet/kubeconfig",
