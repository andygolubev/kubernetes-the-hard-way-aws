...
build {
  ...
  provisioner "file" { // replace
    sources = ["/tmp/kthw-certs/ca.pem",
      "/tmp/kthw-certs/working-node-1-key.pem",
    "/tmp/kthw-certs/working-node-1.pem"] 
    destination = "/etc/kubernetes/certs/"
  }

  provisioner "file" { //replace
    sources = ["/tmp/kthw-certs/working-node-1.kubeconfig",
  ...
  provisioner "file" { //replace
    sources = ["/tmp/kthw-certs/kubelet-config.yaml-1"]
  ...
  provisioner "file" { //replace
    sources     = ["/tmp/kthw-certs/kubelet.service-1"]
  ...
  provisioner "shell" { // replace
    inline = [
      "sudo cp /etc/kubernetes/certs/working-node-1-key.pem /var/lib/kubelet/",
      "sudo cp /etc/kubernetes/certs/working-node-1.pem /var/lib/kubelet/",
      "sudo cp /etc/kubernetes/certs/ca.pem /var/lib/kubernetes/",
      "sudo cp /etc/kubernetes/config/working-node-1.kubeconfig /var/lib/kubelet/kubeconfig",
